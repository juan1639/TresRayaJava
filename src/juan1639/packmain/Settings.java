package juan1639.packmain;

import java.awt.Color;

public class Settings {
	
	// Size de cada casilla (X, O) = 180px
	public static final Integer tileX = 180;
	public static final Integer tileY = 180;
	
	public static final Integer filas = 3;
	public static final Integer columnas = 3;
	
	public static final String FICHA_X = "x";
	public static final String FICHA_O = "o";
	
	public static final Integer nro_casillas9 = 9;
	
	public static final Color negro = new Color(1, 1, 1);
	public static final Color amarillo = new Color(245, 235, 1);
	public static final Color blanco = new Color(247, 247, 247);
	public static final Color azulX = new Color(9, 125, 185);
	public static final Color verdeO = new Color(50, 195, 9);
}
